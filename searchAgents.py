    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                unvis_pos = state[1]
                new_pos = []
                for pos in unvis_pos:
                    if pos != (nextx, nexty):
                        new_pos.append(pos)
                successors.append((((nextx, nexty), tuple(new_pos)), action, 1) )
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in unvisitedCorners:
        dists.append(util.manhattanDistance(currPos, corner))
    return max(dists)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for coord in foodCoords:
        dists.append(mazeDistance(position, coord, problem.startingGameState))
    return max(dists)
